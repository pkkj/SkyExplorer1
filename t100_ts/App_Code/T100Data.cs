        ...
        public static string QueryByOrigin( string year, string origin, string dest, string airline, string queryType, string locale ) {
            ...
            if ( queryType == "iata" ) {
                ...
                if ( lstCoordinate.Length == 4 ) {
                    ...
                    keyword = QueryT100AirportByGeometry( x1, y1, x2, y2, "iata", locale );
                    if ( keyword == "" )
                        return "";
                } else {
                    return "";
            ...
            try {
                ...
                while ( dr.Read() ) {
                    if ( res != "" )
                        res += ",";
                    Airport airport = AirportData.Query( dr[ origin != "" ? "DEST" : "ORIGIN" ].ToString(), locale );
                    Dictionary<string, object> dict = airport.CastToDict();
                    dict[ "SUM_PAX" ] = Convert.ToInt32( dr[ "SUM_PAX" ].ToString() );
                    dict[ "SUM_FREIGHT" ] = Convert.ToInt32( dr[ "SUM_FREIGHT" ].ToString() );
                    string geom = dr[ "GEOM" ].ToString();
                    geom = geom.Replace( "MULTILINESTRING", "" );
                    geom = geom.Replace( "(", "[" );
                    geom = geom.Replace( ")", "]" );
                    dict[ "GEOM" ] = geom;
                    dict[ "DATA_SOURCE" ] = "T100";
                ...
                sql = "SELECT " + fields + " FROM \"CaaSummary\"" + where;
                command = new NpgsqlCommand( sql, conn );
                dr = command.ExecuteReader();
                while ( dr.Read() ) {

                    Airport airport = AirportData.Query( dr[ origin != "" ? "DEST" : "ORIGIN" ].ToString(), locale );
                    if ( airport.CountryEn == Global.CURRENT_COUNTRY )
                        continue;
                    if ( res != "" )
                        res += ",";
                    Dictionary<string, object> dict = airport.CastToDict();
                    ...
                    geom = geom.Replace( "(", "[" );
                    geom = geom.Replace( ")", "]" );
                    dict[ "TOTAL_PAX" ] = dr[ "TOTAL_PAX" ];
                    dict[ "GEOM" ] = geom;
                    dict[ "DATA_SOURCE" ] = "CAA";
                    string json = new JavaScriptSerializer().Serialize( dict );
                    res += json;
                }
            ...
            res = "\"routes\": [" + res + "]";
            string iata = origin != "" ? origin : dest;
            Airport tmpAirport = AirportData.Query( iata, locale );
            if ( tmpAirport == null ) {
                return "";
            }
            string jsonAirport = new JavaScriptSerializer().Serialize( tmpAirport );
            res = "\"fromAirport\": " + jsonAirport + "," + res;
            res = "{" + res + "}";
            return res;
        }
